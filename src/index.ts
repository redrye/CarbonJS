...
export type CalendarType = "jalaali" | "islamic";
export type Input = string | number | Date | Carbon;

export type DateInputArray = [number, number, number, number, number, number, number];

export type BaseUnit = "y" | "year" | "M" | "month" | "d" | "day" |
    "h" | "hour" | "m" | "minute" | "s" | "second" | "ms" | "millisecond";

export type ManipulationUnit = BaseUnit | "years" | "months" | "w" | "week" | "weeks" | "days" |
    "hours" | "minutes" | "seconds" | "milliseconds";

export type CalendarUnit = "jY" | "jM" | "jYear" | "jMonth" | "iY" | "iM" | "iYear" | "iMonth";
...
export interface Carbon {
    ...
    isBetween(from: Input, to: Input): boolean;

    // Calendar specific calculations  
    dayOfYear(calendar?: CalendarType): number;

    isLeapYear(calendar?: CalendarType): boolean;

    weekOfYear(calendar?: CalendarType): number;

    daysInMonth(calendar: CalendarType): number;

    // Relative time
    from(input: Input, withoutSuffix?: boolean): string;

    to(input: Input, withoutSuffix?: boolean): string;
    ...
    // Calendar Getters
    year(calendar: CalendarType): number;

    month(calendar: CalendarType): number;

    day(calendar: CalendarType): number;

    // Arithmetic
    add(value: string | number, unit: ManipulationUnit | CalendarUnit): this;

    subtract(value: string | number, unit: ManipulationUnit | CalendarUnit): this;
}
